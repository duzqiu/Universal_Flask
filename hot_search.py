...
def get_hot_search(limit=10):
    platform_list = [
        "baidu", "toutiao", "bilibili",
        "dy_hot", "dy_plant", "dy_entertain", "dy_society", "dy_sh",
        "sina_hot", "sina_cmnt", "sina_video", "sina_trend", "sina_sport", 
        "sina_ent", "sina_auto", "sina_fashion","sina_travel", "sina_ai"
        ] # 支持的平台列表
    try: 
        result = dict()  # 初始化结果字典
...
@app.route('/api/hot_search/<platform>/<int:limit>', methods=['GET'])
@app.route('/api/hot_search/<platform>', methods=['GET'])
def get_hot_search_by_platform(platform, limit=100):
    try: 
        result = dict()  # 初始化结果字典
        cursor = mongo_server.get_data(platform, limit=limit) # 获取指定平台的数据
        result[platform] = list(cursor) # 将游标转换为列表
